...
export function computeAutoLayout(
  ...
  for (const tableEl of autoTables) {
          ...
          if (allFitFull) {
            ...
            workElements = workElements.map((el) => {
              ...
              if (belowIdsFull.has(el.id)) {
                const gap = Math.max(0, el.y - origTableBottom2);
                return { ...el, y: naturalBottom35 + gap };
              }
              return el;
            });
          } else {
    ...
    // Use measured average row height for continuation fit calculation when available.
        ...
        if (allFit2) {
          ...
          workElements = workElements.map((el) => {
            ...
            if (belowIds.has(el.id)) {
              const gap = Math.max(0, el.y - origTableBottom);
              return { ...el, y: naturalBottom2 + gap };
            }
            return el;
          });
        } else {
    ...
    while (sliceStart < allRows.length) {
      ...
      // Last slice has no overflow indicator; non-last pages show the indicator badge.
