...
export default function IssuesPage() {
  ...
  // hard-coded `/issues` — the route now lives under the slug (ADR 0011 §4).
  const pathname = usePathname();
  // The workspace comes from the URL: the `[workspaceSlug]` layout already
  // resolved this slug (cache-shared query), so `workspace` is available on
  // first render (same pattern as the analytics page, part A).
  ...
  const { data, isLoading } = useIssues({
    ...
    // ADR 0011 §2: scope to this workspace's readable projects; personal
    // issues never appear under a workspace URL.
