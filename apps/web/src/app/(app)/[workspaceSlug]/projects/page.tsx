...
export default function ProjectsPage() {
  ...
  // Strict workspace scoping (ADR 0006): this list shows only projects whose
  // workspaceId matches the one the URL names — a project you own in another
  // workspace deliberately does NOT appear here.
  const ws = useWorkspaceHref();
  const { current } = useCurrentWorkspace();
  const { data: projects = [], isLoading } = useProjectsInWorkspace(
    current?.id ?? null,
  );
    ...
    <>
      ...
      <ProjectModal
        ...
        onClose={() => {
          setCreating(false);
          setEditing(null);
          if (wantNew) router.replace(ws('/projects'));
        }}
      />
