...
export default function TimelinePage() {
  ...
  // Query params — workspaceId (ADR 0011 §2) scopes the feed to activity on
  // this workspace's readable projects.
  const queryParams = useMemo(() => {
    const p: Record<string, string | number> = { limit: 200 };
    if (from) p.from = from;
    if (to)   p.to   = to;
    if (!isAdmin && me) p.userId = me.id;
    if (workspace) p.workspaceId = workspace.id;
    return p;
  }, [from, to, isAdmin, me, workspace]);
