...
import { useNoteFolderGrants, useNoteMutations } from '@/hooks/use-notes';
import { useUsers } from '@/hooks/use-users';
import { useAuthStore } from '@/stores/auth-store';
import { cn } from '@/lib/utils';
import type { GrantLevel, GrantRole, NoteFolder } from '@/schemas/note';

const LEVEL_OPTIONS: { value: GrantLevel; label: string; hint: string }[] = [
  { value: 'read', label: 'Read', hint: 'View notes in this folder' },
  { value: 'upload', label: 'Create', hint: 'Read + create own notes' },
  { value: 'edit', label: 'Edit', hint: 'Manage any note in folder' },
...
export function ShareFolderModal({
  ...
  const { data: grants = [] } = useNoteFolderGrants(folder?._id ?? null);
  const { data: users = [] } = useUsers();
  const { setGrant, removeGrant } = useNoteMutations();
