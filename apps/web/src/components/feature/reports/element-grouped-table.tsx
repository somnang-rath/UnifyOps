...
export function ElementGroupedTable({ element }: Props) {
  ...
  const startGI    = p.startGroupIdx ?? 0;
  const endGI      = p.endGroupIdx;   // undefined = all remaining
  ...
  for (const siteRow of rawData) {
    ...
    if (!groupIdx.has(gKey)) {
      groupIdx.set(gKey, allGroups.length);
      allGroups.push({ key: gKey, firstRow: siteRow, sites: [], totalDetailRows: 0 });
    }
    const grp = allGroups[groupIdx.get(gKey)!];
    grp.sites.push({ siteRow, details });
    grp.totalDetailRows += Math.max(1, details.length);
  }

  // Apply startGroupIdx / endGroupIdx slice for pagination
  const groups = allGroups.slice(startGI, endGI);
  ...
  return (
    ...
    >
      {/* continuation badge */}
      {p.isContinuation && (
        <div className="flex-shrink-0 px-2 py-0.5 text-[9px] font-semibold" style={{ background: 'rgba(99,102,241,0.08)', borderBottom: '1px dashed #6366f1', color: '#6366f1' }}>
          ↩ Continued from previous page
        </div>
      )}
      {/* datasource URL badge */}
      {p.dataUrl && !p.isContinuation && (
