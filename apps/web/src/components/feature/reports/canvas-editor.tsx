...
const SHORTCUTS = [
  { keys: ['Arrow', 'keys'],       desc: 'Nudge element 1px' },
  { keys: ['Shift', '+', 'Arrow'], desc: 'Nudge element 10px' },
...
export function CanvasEditor({ template, onChange }: Props) {
  ...
  const deletePage = useCallback((idx: number) => {
    ...
    onChange({ pages: newPages, elements: newElements });
  ...
  const duplicatePage = useCallback((idx: number) => {
    ...
    onChange({ pages: [...pages, newPage], elements: [...allElements, ...newEls] });
  ...
  return (
    ...
    <div className="flex flex-col flex-1 overflow-hidden" onContextMenu={(e) => e.preventDefault()}>
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        >
          ...
          {leftTab === 'page' && (
            <div className="min-w-[240px] flex-1 overflow-y-auto scroll-smooth">
              <PageSetupPanel template={template} onChange={onChange} />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div
            ...
            onKeyDown={(e) => {
              // Prevent the overflow-scroll container from consuming arrow keys
              // when elements are selected. The window handler in useReportController
              // fires on the same event (bubbling) and does the actual nudge.
              const isArrow = e.key === 'ArrowLeft' || e.key === 'ArrowRight' ||
                              e.key === 'ArrowUp'   || e.key === 'ArrowDown';
              if (isArrow && ctrl.selectedIds.length > 0 && !e.ctrlKey && !e.metaKey &&
                        ...
                        onResizeEnd={({ target }) => {
                          ...
                          if (g && g.members.length > 0) {
                            ...
                            if (selEl) {
                              ...
                              if (selEl.type === 'table' && base.autoPageBreak &&
                            ...
                            if (selEl) {
                              ...
                              if (selEl.type === 'table' && sp.autoPageBreak && !sp.isContinuation && !sp.autoGenerated && sp.autoOriginalH !== undefined) {
      ...
      {/* ── Bottom bar: Zoom + Snap + Shortcuts + Page tabs ─────────────── */}
