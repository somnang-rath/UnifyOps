...
import { ElementChartMemo } from './element-chart';
...
export function CanvasEditor({ template, onChange, refreshDatasourcesRef, prepareForExportRef, getLatestTemplate }: Props) {
  ...
  useEffect(() => {
    ...
    // Update the ref to the RESULT elements' sig, not the input sig. This prevents
    // the next render (triggered by onChange below) from re-entering this effect,
    // which would cause a "Maximum update depth exceeded" loop.
    ctrl.autoLayoutSigRef.current = result
      ? autoLayoutSig(result.elements, canvasH, mt, mb, fh, hh)
      : sig;
  ...
  const prepareForExport = useCallback(async () => {
    ...
    try {
      ...
      // Build the hint map: start from the live DOM-measured hints, then fall back
      // to the saved endRow on each source table element.  This guards against the
      // case where actualFitHintsRef was cleared (e.g. after resetLayoutSigs) before
      // this call — without a hint the formula uses stretchedH and may overestimate
      // rows-per-page, causing Case 2 ("all rows fit") to fire and strip continuation
      // pages from the saved template, which then produces a wrong PDF.
      const effectiveHints: Record<string, number> = { ...ctrl.actualFitHintsRef.current };
      for (const el of nextElements) {
        if (el.type !== 'table') continue;
        const p = el.props as Record<string, unknown>;
        if (p.isContinuation || p.autoGenerated) continue;
        if (effectiveHints[el.id] == null) {
          const savedEndRow = p.endRow as number | undefined;
          if (savedEndRow != null) effectiveHints[el.id] = savedEndRow;
        }
      }

      const layoutResult    = computeAutoLayout(freshTemplate, effectiveHints);
      if (layoutResult) {
        // Pre-arm the sig ref with the RESULT sig so the auto-layout useEffect bails
        // out immediately on the next render and doesn't re-run layout a second time.
        // Without this, the effect sees a new sig (result differs from current), fires
        // again, and the cycle can exceed React's update depth limit on re-preview.
        const rPs = PAGE_CANVAS_SIZES[freshTemplate.pageSize] ?? PAGE_CANVAS_SIZES.A4;
        const rH  = freshTemplate.orientation === 'landscape' ? rPs.w : rPs.h;
        const rMt = freshTemplate.margins?.top    ?? 0;
        const rMb = freshTemplate.margins?.bottom ?? 0;
        const rFh = freshTemplate.footer?.enabled ? (freshTemplate.footer?.height ?? 0) : 0;
        const rHh = freshTemplate.header?.enabled ? (freshTemplate.header?.height ?? 0) : 0;
        ctrl.autoLayoutSigRef.current = autoLayoutSig(layoutResult.elements, rH, rMt, rMb, rFh, rHh);
...
function renderElement(
  ...
  switch (el.type) {
    ...
    case 'chart':        return <ElementChartMemo element={el} />;
